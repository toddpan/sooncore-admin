		...
		foreach($template_groups as $templateUUID=>$users){
			...
			foreach($users as $k=>$user){
				if(!empty($user['sellingProducts'])){
					list($flag, $request_id) = $this->sendBossRequest($rst['customer_code'], $rst['site_id'], array_column($users, 'id'), 'disable', $templateUUID, $user['sellingProducts']);
					if(! $flag){
			...
			//剩下的一起发送
			if(count($users) > 0){
				list($flag, $request_id) = $this->sendBossRequest($rst['customer_code'], $rst['site_id'], array_column($users, 'id'), 'disable', $templateUUID);
				if(! $flag){
					$failed_request_ids[] = $request_id;
				}
			}
